...
const SellerOrdersTab = lazy(() =>
  import('./modules/seller-orders').then((m) => ({ default: m.SellerOrdersTab }))
);
const SellerNegotiationTab = lazy(() =>
  import('./modules/negotiation').then((m) => ({ default: m.SellerNegotiationTab }))
);
      ...
      <main className="flex-1 min-w-0 w-full max-w-max-width mx-auto px-4 lg:px-10 py-6 pb-24 sm:pb-6">
        ...
        <Suspense fallback={<FullScreenLoading />}>
          ...
          {activeTab === TAB_KEYS.ORDERS && (
            <SellerOrdersTab onToast={showToast} />
          )}
          {activeTab === TAB_KEYS.NEGOTIATION && (
            <SellerNegotiationTab products={products} onToast={showToast} />
          )}
